...
func IntToHex(val *big.Int) string {
	hexStr := val.Text(16)
	if len(hexStr) < 32 {
...
// ToModifyByCloidWire converts an OrderRequest to a ModifyByCloidWire using the provided AssetInfo.
func (req *OrderRequest) ToModifyByCloidWire(info AssetInfo) ModifyOrderByCloidWire {
	var cloid string
	if req.Cloid != "" {
		cloid = req.Cloid
	}
	return ModifyOrderByCloidWire{
		Cloid: cloid,
		Order: req.ToWire(info),
	}
}
...
// SizeToFloat calls SizeToWire for consistent rounding,
// then parses the result back to float64.
func SizeToFloat(x float64, szDecimals int) float64 {
	wireStr := SizeToWire(x, szDecimals)
	val, _ := strconv.ParseFloat(wireStr, 64)
	return val
}
